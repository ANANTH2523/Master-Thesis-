...
% -------------------
\abstract
\todo[inline]{Do not forget to read again the \textbf{genAI policy} for degree projects:

\url{https://bth.instructure.com/courses/3493/pages/genai-policy}

The rules also apply to the work you conduct when producing this document.}
...
% ---------------------------
\das
\todo[inline]{The purpose of the data availability statement is to enable other researchers to replicate your work and build upon your results. The published material should be sufficient for another researcher to understand and, where legally and ethically possible, reproduce the analysis reported in the thesis. The repository should therefore include appropriate documentation, such as a README file describing the folder structure, file contents, data collection and processing steps, analysis scripts, software dependencies, and instructions for reproducing the results.

With \emph{data}, we mean any material that supports the chain of evidence from the results, analysis, to your conclusions. This may include, but is not limited to, a systematic literature protocol and data extraction sheet, mined source code repositories, raw and coded interview transcripts.

With \emph{source code}, we mean any code you developed to perform the analysis on the data you collected, as well as any other contribution that you describe in your thesis.

If only parts of the data or source code can be made available, state clearly which parts are available, which parts are restricted, and why. When raw data cannot be shared, consider publishing anonymized, aggregated, derived, or synthetic data, together with protocols, codebooks, analysis scripts, or metadata that support transparency without violating confidentiality, GDPR, non-disclosure agreements, or intellectual property rights.
In case your data or source code cannot be published due to non-disclosure agreements or the violation of current or potential future intellectual property rights, you \emph{must} state this explicitly here and provide details as to why the data or source code cannot be disclosed. 

Please contact your supervisor to discuss how to prepare the data or source code for publication, i.e. how to anonymize data and obfuscate proprietary or personal information. 
The data and source code should be archived in a persistent repository and should include a clear license, citation information, and version information. A GitHub repository may be used for development, but the final thesis version should preferably use a repository that provides a permanent identifier, such as Zenodo or figshare.}
\cleardoublepage
% -------------------
...
% Chapter files are in thesis/chapters/ — \input them here as you write content:
...
% All references are in: thesis-refs.bib (same directory as this file)
